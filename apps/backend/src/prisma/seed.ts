...
import {
  getGeneralAgentPromptPath,
  getGeneralAgentTemplatePath,
  getSalesAgentPromptPath,
  getSalesAgentTemplatePath,
} from '../lib/paths.js';
...
async function main() {
  ...
  const defaults: Record<string, unknown> = {
    ...
    // Align with agent-config DEFAULTS — tools + first prompt are general.
    agent_config: {
      mode: 'general',
    },
  };
  ...
  // Seed system prompt from general-agent.txt (if no prompts exist yet).
  ...
  // Legacy sales-agent.txt is a last-resort fallback for older workspaces.
  ...
  if (promptCount === 0) {
    ...
    if (!promptPath) {
      console.warn('Could not seed system prompt: no general/sales agent template found.');
    } else {
      try {
        ...
        await prisma.systemPrompt.create({
          data: {
            ...
            content,
            author: 'human',
            changeSummary: 'Initial system prompt (general agent template)',
            isActive: true,
          },
        });
