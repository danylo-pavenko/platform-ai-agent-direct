...
export function buildFalseBookingConfirmNudge(): string {
  return (
    ...
    `Якщо чогось бракує — НЕ стверджуй запис і НЕ викликай book з порожнім phone: залиш факти (ціна/тривалість) і коротко попроси лише відсутнє поле. ` +
    `Не пиши знову «чекаємо тебе / записала / бачимось / в обробці».`
  );
}
