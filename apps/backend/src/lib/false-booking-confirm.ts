...
const BOOKING_CONFIRM_RE =
  /чекаємо\s+(на\s+)?(тебе|вас)|чекаємо\s+(завтра|сьогодні|післязавтра)|будемо\s+чекати\s+(тебе|вас)|записую\s+(тебе|вас)|записала?\s+(тебе|вас)|записав\s+(тебе|вас)|ти\s+записан|вас\s+записан|ви\s+записан|запис\s+(підтверджено|створено|оформлено|готовий)|бачимось\s+(завтра|сьогодні|о\s+\d)|забронювала?\s+(тебе|вас|на\s+)/i;
