...
/**
 * Register instagramUserId on the platform webhook hub so POST /webhooks/instagram
 * routes events to this tenant's localhost:apiPort.
 */
export function syncWebhookRoutingToHub(
  ...
  fetch(syncUrl, {
    ...
    body: JSON.stringify({ instagramUserId: igUserId, facebookAppSecret }),
    signal: AbortSignal.timeout(8_000),
  })
    .then(async (res) => {
      if (res.ok) {
        log.info({ igUserId }, 'Webhook hub routing synced');
      } else {
