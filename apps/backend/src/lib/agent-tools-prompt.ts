...
export function formatAgentToolsPrompt(tools: ToolDefinition[]): string {
  ...
  if (names.has('search_services')) {
    rules.push(
      ...
      'Ціна послуги залежить від рівня майстра (назви грейдів — лише як у результаті tools / каталозі, напр. Молодший майстер / Майстер / Топ майстер / Преміум майстер). Не вигадуй грейд. Молодший/Майстер можуть не робити всі послуги — якщо tool каже «недоступно для цього майстра», запропонуй інший рівень.',
