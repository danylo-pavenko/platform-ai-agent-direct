...
export function formatAgentToolsPrompt(
  ...
  if (names.has('get_available_slots')) {
    rules.push(
      ...
      'Два+ майстри з однаковим імʼям у слотах/каталозі — розрізняй за positions у підписі або повним [master_id=UUID]; у book_appointment завжди ПОВНИЙ UUID з блоку «Запропоновані вікна» або з ОСТАННЬОГО get_available_slots для ЦІЄЇ послуги, ніколи імʼя і ніколи перші 8 символів id.',
