...
export function formatAgentToolsPrompt(
  ...
  if (names.has('get_available_slots')) {
    rules.push(
      ...
      'Якщо в попередній відповіді або в блоці «Запропоновані вікна» уже є конкретні години і клієнт обрав одну з них (або «перший/другий слот», «як ти писала») — не роби новий get_available_slots і не кажи що вікон немає, доки book_appointment не повернув SLOT_NOT_AVAILABLE / TIME_CONFLICT / MASTER_DAY_CLOSED або клієнт не змінив послуги/дату/майстрів. Пауза на ПІБ/телефон не вимагає свіжого lookup.',
