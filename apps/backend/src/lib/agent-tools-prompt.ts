...
export function formatAgentToolsPrompt(
  ...
  if (names.has('book_appointment')) {
    rules.push(
      'Коли клієнт підтвердив слот (дата/час/послуга/філія за потреби) і аргументи book_appointment уже відомі з цієї розмови або з блоку «Вже відомо про клієнта» — у ТІЙ САМІЙ відповіді ОБОВʼЯЗКОВО виклич book_appointment (ПІБ, телефон, дата ДД.ММ.РРРР, час, services[{id,duration_min,master_id якщо різні майстри, start_time якщо різні години старту}]). Усі послуги одного візиту — одним масивом services[] (одне замовлення). Один майстер на всі послуги — top-level master_id. Паралельно до різних майстрів — master_id на КОЖНОМУ рядку services (з get_available_slots); інакше обидві послуги сядуть на одного і CRM дасть TIME_CONFLICT. Різні години між різними майстрами (10:30 і 11:00) — start_time на КОЖНОМУ рядку. Кілька послуг ОДНОГО майстра (комплекс + укріплення + дизайн) — окремі рядки з duration_min з каталогу; НЕ став усім той самий start_time (платформа вибудує їх підряд: 10:00 → +115 → +60).',
      'Не викликай book_appointment з порожнім phone чи customer_name («», «немає», заглушка). Бракує поля, яке schema tool вимагає, і його немає в профілі — спитай лише його; слоти можна показувати без контактів. Якщо в блоці «Вже відомо» є рядок «Імʼя:» або «Телефон:» — використай, не перепитуй. Якщо є «Назва профілю Instagram» — це шапка, не імʼя: спитай як звертатись, update_client_info(full_name), не став її в customer_name. Додаткові поля з системного промпту тенанта (дата народження тощо) запис не блокують, якщо тенант не сказав інакше.',
  ...
  if (names.has('get_available_slots')) {
    rules.push(
      ...
      'Два+ майстри з однаковим імʼям у слотах/каталозі — розрізняй за positions у підписі або повним [master_id=id]; у book_appointment завжди повний id з блоку «Запропоновані вікна» або з ОСТАННЬОГО get_available_slots для ЦІЄЇ послуги (UUID BeautyPro або число CleverBOX), ніколи імʼя і ніколи обрізаний фрагмент id.',
      ...
      'Якщо в попередній відповіді або в блоці «Запропоновані вікна» уже є конкретні години і клієнт обрав одну з них (або «перший/другий слот», «як ти писала») — не роби новий get_available_slots і не кажи що вікон немає, доки book_appointment не повернув SLOT_NOT_AVAILABLE / TIME_CONFLICT / MASTER_DAY_CLOSED або клієнт не змінив послуги/дату/майстрів. Пауза на ПІБ/телефон не вимагає свіжого lookup. services[].id і master_id бери з того блоку (повний id на рядку години).',
      'Не супереч останньому get_available_slots у тій самій розмові: якщо tool показав вільні години на дату — не пиши що місць немає (і навпаки) без нового виклику tool.',
      ...
      'Клієнту називай майстрів лише імʼям (без прізвища). Якщо в слотах є підпис з напрямом у дужках — можна коротко: «Анастасія (манікюр)».',
      'Після [book_appointment] ok — не пропонуй інші години в тій самій відповіді і не викликай get_available_slots знову, доки клієнт не попросить перенести.',
  ...
  if (names.has('update_client_info')) {
    rules.push(
      ...
      'full_name — рядок «Імʼя:» з профілю або як клієнт представився в чаті. Не копіюй «Назву профілю Instagram» (шапку).',
      'ПІБ і телефон часто приходять окремими бульбашками Instagram одразу після вибору часу — це та сама відповідь. Якщо в історії або в поточному блоці вже є імʼя/телефон — виклич update_client_info і не проси повторити.',
    );
  }
