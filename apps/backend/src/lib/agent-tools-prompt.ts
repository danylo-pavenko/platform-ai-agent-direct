...
export function formatAgentToolsPrompt(
  ...
  if (names.has('book_appointment')) {
    rules.push(
      ...
      'Не викликай book_appointment з порожнім phone чи customer_name («», «немає», заглушка). Бракує поля, яке schema tool вимагає, і його немає в профілі — спитай лише його; слоти можна показувати без контактів. Якщо в профілі вже є Телефон або Імʼя — використай, не перепитуй. Додаткові поля з системного промпту тенанта (дата народження тощо) запис не блокують, якщо тенант не сказав інакше.',
