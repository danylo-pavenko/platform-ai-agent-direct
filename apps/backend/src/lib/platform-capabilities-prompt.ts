...
- Два майстри з однаковим імʼям — у слотах підпис з positions / повний [master_id=id]; у tools завжди повний id з «Запропоновані вікна» або останнього get_available_slots, не імʼя і не обрізаний фрагмент.
...
- BeautyPro і CleverBOX: book_appointment потребує повний service_id / master_id з «Запропоновані вікна» (UUID або число). Обрізаний префікс чи імʼя платформа розгортає з оферу/каталогу, якщо унікально; інакше INVALID_CRM_ID, без локального запису і без POST. Слово reschedule — не id послуги.
...
- Запропоновані години з get_available_slots платформа зберігає в розмові (~2 год) і підставляє як «Запропоновані вікна» з повними service_id / master_id на кожній годині. Якщо клієнт обрав одну з них — book_appointment без нового lookup, з id з цього блоку. Новий get_available_slots лише коли змінили послугу/дату/майстра або book повернув SLOT_NOT_AVAILABLE / TIME_CONFLICT / MASTER_DAY_CLOSED. Пауза на ПІБ/телефон не вимагає свіжих слотів.
- Історія Claude в turn — цивільний день tenant цієї розмови (або від createdAt, якщо тред почався сьогодні). Якщо сьогодні вже було завершене замовлення/візит — від повідомлень після нього. Не last-30.
...
- book_appointment: можна передати services[].price з цитати слотів; CRM BeautyPro рахує ціну сам — поле для локального запису/клієнта. services[].name — людська назва; платформа підставляє з каталогу, якщо агент передав лише id (клієнту ніколи UUID).
- У промпт клієнта інжектяться «Найближчі записи» (локальні confirmed/synced на сьогодні+кілька днів). Повідомлення «запізнююсь / я тут / через N хв» — про той візит, не новий book_appointment.
...
- Перший вхід від нового IG-користувача — платформа підтягує до 20 останніх повідомлень з Instagram у цей діалог (контекст до бота). На ПЕРШІЙ відповіді бота все одно представся імʼям/роллю зі системного промпту тенанта в тій самій репліці, що й відповідь по суті. Не питай «хто ви?», якщо в історії вже є листування. «Не вітайся знову» діє лише в цьому календарному дні салону (або під час поточного оформлення замовлення/запису) — навіть якщо клієнт написав «добрий ранок». Нова календарна доба в тому ж UUID (вчора 23:00 → сьогодні 09:00) = новий день: у історії буде «Новий календарний день салону»; вітайся знову лише якщо це прописано в системному промпті тенанта. Після sessionFreshnessDays (типово 14) тиші платформа відкриває НОВУ розмову — тоді знову представся. У історії Claude є дата/час і хто писав (менеджер ≠ бот); довга пауза (≥ freshness) = можливе нове звернення, не продовжуй старий конфлікт.
- Telegram менеджерам під час handoff: одна картка ескалації в групи (імʼя або @username, без IGSID і без «Хід агента»). Сервісний дамп ходу агента — лише в особистий чат з ботом після /login. Повідомлення клієнта в handoff не форвардяться в Telegram. Одне SLA-нагадування, якщо клієнт чекає довше managerSlaHoursBusiness робочих годин без відповіді менеджера. TTL без відповіді менеджера повертає бота на наступному inbound, замовлення не закриває.
- Адмінка «Розмова»: кнопки Надіслати реквізити / Відповісти по суті / Оформити замовлення. Реквізити — agent_config.paymentRequisites. «Відповісти по суті» залежить від режиму: sales — каталог/доставка/контакти (без collect_order); booking — слоти/book якщо година вже підтверджена; general — за наміром. Оформити може створити collect_order / create_local_order навіть у handoff; платіжка → note «ПОТРІБНЕ ПІДТВЕРДЖЕННЯ ЛЮДИНИ».
