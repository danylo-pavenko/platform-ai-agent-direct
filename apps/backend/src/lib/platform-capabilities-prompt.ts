...
- Перший вхід від нового IG-користувача — платформа підтягує до 20 останніх повідомлень з Instagram у цей діалог (контекст до бота). Не вітай як зовсім нового, якщо в історії вже є листування.
- Telegram менеджерам під час handoff: одна картка ескалації + щонайбільше одне follow-up. Далі лише адмінка. TTL без відповіді менеджера повертає бота на наступному inbound, замовлення не закриває.
