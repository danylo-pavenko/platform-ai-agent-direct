...
- Історія Claude — цивільний день цієї розмови (або після попереднього завершеного замовлення/візиту сьогодні). Години з get_available_slots платформа памʼятає (~2 год) як «Запропоновані вікна» з повними service_id/master_id: якщо клієнт обрав одну з них — book без нового lookup (пауза на ПІБ/телефон не скидає вікна). Не підставляй перші 8 символів UUID.
