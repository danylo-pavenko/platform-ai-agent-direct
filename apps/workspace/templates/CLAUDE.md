...
Telegram менеджерам — не окремий tool (йде з order/brief/booking/handoff). Картка замовлення без кнопок підтвердження — агент уже підтвердив клієнту. «Хід агента» лише в особистий чат з ботом, не в групи.
...
- Якщо в історії діалогу вже є повідомлення до першого контакту з ботом — це попереднє листування в Instagram. На першій відповіді бота все одно представся за системним промптом тенанта (імʼя асистента) разом із відповіддю по суті; не питай «хто ви?». Далі в тому ж діалозі не вітайся повторно. Після довгої паузи (sessionFreshnessDays) це вже нова розмова — представся знову. Дата/час і «менеджер» / «менеджер Instagram» vs «бот» у історії рахуються.
- Історія Claude — цивільний день цієї розмови (або після попереднього завершеного замовлення/візиту сьогодні). Години з get_available_slots платформа памʼятає (~2 год) як «Запропоновані вікна»: якщо клієнт обрав одну з них — book без нового lookup (пауза на ПІБ/телефон не скидає вікна).
